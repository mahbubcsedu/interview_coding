...
public class NumberOfIslands200 {
	...
	/*
	 * Fastest on leetcode 3ms
	 */
	public int numIslandsFastest(char[][] grid) {
        int m = grid.length;
        if(m == 0) return 0;
    ...
    public static void merge(char[][] m, int i, int j){
        m[i][j] = '0';
        if(i > 0 && m[i-1][j] == '1'){merge(m, i-1, j);}
        if(j > 0 && m[i][j-1] == '1'){merge(m, i, j-1);}
        if(i+1 < m.length && m[i+1][j] == '1'){merge(m, i+1, j);}
        if(j+1 < m[i].length && m[i][j+1] == '1'){merge(m, i, j+1);}
        return;
    }
